...
object Facts {
  ...
  def main(args: Array[String]) {
    ...
    val weatherWithTime = spark.read.textFile(s"/user/$username/labs/spark/uk-traffic/weather.txt")
      ...
      .withColumn("timestamp", to_timestamp($"timestamp", "dd/MM/yyyy HH:mm")) //todo w pliku jest bez sekund
    ...
    val allTrafficWithTime = allTraffic.flatMap(r => vehicle_types.zipWithIndex.map(v =>
      (r.getInt(0), r.getTimestamp(3), r.getInt(4), r.getString(5), r.getInt(v._2 + 17), v._1, r.getString(6), r.getString(7))))
